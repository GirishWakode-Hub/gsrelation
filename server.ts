...
async function startServer() {
  ...
  // ==========================================
  // ADITI (CO-FOUNDER) PHOTO UPLOAD & SYNC API
  // ==========================================
  app.post('/api/upload-aditi-photo', (req: Request, res: Response) => {
    try {
      const { image } = req.body;
      if (!image || typeof image !== 'string') {
        return res.status(400).json({ success: false, error: 'No image data provided' });
      ...
      const publicPath = path.join(imagesDir, 'rahi.png');
      fs.writeFileSync(publicPath, buffer);

      const distImagesDir = path.join(process.cwd(), 'dist', 'images');
      if (fs.existsSync(distImagesDir)) {
        fs.writeFileSync(path.join(distImagesDir, 'rahi.png'), buffer);
      }

      console.log(`[API] Aditi photo updated successfully (${buffer.length} bytes)`);
      return res.json({
        success: true,
        url: `/images/rahi.png?v=${Date.now()}`,
        message: 'Aditi photo updated successfully on server',
      });
    } catch (err: any) {
      console.error('Error saving Aditi photo:', err);
      return res.status(500).json({ success: false, error: err.message || 'Failed to save photo' });
    }
  });
...
Key Facts & Corrective Actions Taken: ${payload.actionsTaken || 'Immediate internal investigation launched'}`;
      ...
      res.json({
        result: text,
        isFallback: false,
        disclaimer: 'Generated by GSRelation Engine via Gemini 3.8.',
      });
    } catch (err: any) {
  ...
  // ==========================================
  // AI STRATEGIC CONSULTATION ASSISTANT
  // ==========================================
  function extractJsonFromGemini(rawText: string): any {
  ...
  app.post('/api/ai/consultation-assist', async (req: Request, res: Response) => {
    const { action, sentence, rawText, company, service, name, goals, preferredDate, preferredTime } = req.body;
    try {
      const client = getGeminiClient();

      if (!client) {
        const fallback = generateFallbackConsultationAssist(action, req.body);
        return res.json({ success: true, data: fallback, isFallback: true });
      }

      if (action === 'smart_fill') {
        const prompt = `You are an elite Indian PR strategist at GSRelation.
Analyze the following sentence from an executive looking to book a strategic PR consultation:
"${sentence}"

Extract or infer the following information and output pure, valid JSON with NO markdown formatting:
{
  "company": "Estimated company name or clear placeholder",
  "service": "One of: 'Strategic Media Relations & Placement', 'Crisis Communications & Issues Management', 'Thought Leadership & Executive Positioning', 'Brand Narrative & Strategic Messaging', 'Financial PR, M&A & Investor Relations', 'Digital PR, Social Amplification & Media Intelligence'",
  "goals": "A polished, media-ready 2-3 sentence strategic goal statement suitable for Indian business publications (ET, Mint, NDTV)",
  "targetOutlets": ["3-4 top Indian business/tech media outlets such as The Economic Times, Mint, CNBC-TV18, YourStory, NDTV Profit"],
  "suggestedAngle": "A compelling 1-line strategic angle or hook for journalists"
}`;
        ...
        try {
          ...
          console.warn('JSON parsing error in smart_fill:', pErr, text);
          const fallback = generateFallbackConsultationAssist(action, req.body);
          return res.json({ success: true, data: fallback, isFallback: true });
        }
      } else if (action === 'refine_goals') {
        const prompt = `You are an elite PR director at GSRelation India.
Refine the following rough notes/goals into an authoritative, strategic PR goal statement suitable for senior media directors:
Company: ${company || 'Enterprise Innovator'}
Practice Area: ${service || 'Strategic Media Relations & Placement'}
Rough Notes: "${rawText || 'Seeking tier-1 media presence and founder visibility.'}"

Output pure, valid JSON with NO markdown formatting:
{
  "refinedGoals": "An authoritative, polished 2-4 sentence executive overview outlining media objectives, key proof points, target outcomes in Indian Rupees or milestones, and Tier-1 press expectations.",
  "targetOutlets": ["3-4 specific Indian publications like The Economic Times, Mint, CNBC-TV18, Business Standard"],
  "suggestedAngle": "A sharp, newsworthy hook connecting this milestone to macro industry trends"
}`;
        ...
        try {
          ...
          console.warn('JSON parsing error in refine_goals:', pErr, text);
          const fallback = generateFallbackConsultationAssist(action, req.body);
          return res.json({ success: true, data: fallback, isFallback: true });
        }
      } else if (action === 'generate_dossier') {
        const prompt = `You are an elite Managing Partner at GSRelation India preparing an Executive Consultation Strategic Briefing Dossier.
Executive Name: ${name || 'Executive Leader'}
Company: ${company || 'Client Enterprise'}
Practice Area: ${service || 'Strategic Media Relations & Placement'}
Strategic Goals: ${goals || 'Scale national media presence and launch milestone.'}
Scheduled Window: ${preferredDate || 'Upcoming Session'} at ${preferredTime || '10:00 AM IST'}

Output pure, valid JSON with NO markdown formatting:
{
  "executiveSummary": "A 2-sentence crisp synthesis of the strategic opportunity.",
  "agendaItems": [
    "Discussion item 1: Narrative differentiation and hard proof points",
    "Discussion item 2: Tier-1 embargo sequencing across Mumbai, Delhi & regional bureaus",
    "Discussion item 3: Broadcast interview readiness (CNBC-TV18 / NDTV Profit)",
    "Discussion item 4: Crisis mitigation and leak containment protocols"
  ],
  "targetMediaPlacements": [
    "The Economic Times Tech / Corporate Desk",
    "Mint Prime Business Feature",
    "CNBC-TV18 Broadcast Feature",
    "YourStory / Inc42 Innovation Spotlight"
  ],
  "recommendedAngle": "The core strategic angle recommended for maximum authority",
  "embargoTimeline": "Suggested 3-phase timeline: T-14 Days (Asset Preparation), T-48 Hours (Exclusive Embargo Briefing), Day 0 (National Syndication & Broadcast)",
  "preCallChecklist": [
    "High-resolution executive portrait & leadership bio",
    "Audited milestones or funding confirmation in ₹ Crores",
    "List of reference clients or institutional partners"
  ]
}`;
        ...
        try {
          ...
          console.warn('JSON parsing error in generate_dossier:', pErr, text);
          const fallback = generateFallbackConsultationAssist(action, req.body);
          return res.json({ success: true, data: fallback, isFallback: true });
        }
      } else {
        return res.status(400).json({ success: false, error: 'Invalid consultation action' });
      }
    } catch (err: any) {
      console.error('Error in /api/ai/consultation-assist:', err);
      const fallback = generateFallbackConsultationAssist(action, req.body);
      return res.json({ success: true, data: fallback, isFallback: true });
    }
  });
...
function generateFallbackConsultationAssist(action: string, payload: any): any {
  const company = payload.company || 'Enterprise Leader';
  const service = payload.service || 'Strategic Media Relations & Placement';
  const sentence = payload.sentence || payload.rawText || '';

  if (action === 'smart_fill') {
    let detectedCompany = company !== 'Enterprise Leader' && company ? company : '';
    if (!detectedCompany && sentence) {
      const match = sentence.match(/(?:at|for|from|name is|company is|startup is|we are|called)\s+([A-Z][A-Za-z0-9\s&]+?)(?:\s+(?:in|launching|raising|planning|with|next|is|and|\.))/i);
      if (match && match[1]) {
        detectedCompany = match[1].trim();
      } else {
        const words = sentence.split(' ');
        if (words.length > 2) detectedCompany = words.slice(0, 2).join(' ') + ' Innovations';
        else detectedCompany = 'Pinnacle Enterprise';
      }
    }

    let detectedService = 'Strategic Media Relations & Placement';
    const lower = sentence.toLowerCase();
    if (lower.includes('crisis') || lower.includes('breach') || lower.includes('rumor') || lower.includes('controversy')) {
      detectedService = 'Crisis Communications & Issues Management';
    } else if (lower.includes('funding') || lower.includes('ipo') || lower.includes('m&a') || lower.includes('series') || lower.includes('investor') || lower.includes('crore')) {
      detectedService = 'Financial PR, M&A & Investor Relations';
    } else if (lower.includes('ceo') || lower.includes('thought leadership') || lower.includes('founder') || lower.includes('keynote')) {
      detectedService = 'Thought Leadership & Executive Positioning';
    } else if (lower.includes('narrative') || lower.includes('brand') || lower.includes('repositioning')) {
      detectedService = 'Brand Narrative & Strategic Messaging';
    }

    return {
      company: detectedCompany || 'Apex Systems India',
      service: detectedService,
      goals: `Orchestrate a national media campaign around upcoming operational milestones, securing exclusive business features across Tier-1 Indian press (The Economic Times, Mint) and high-impact executive broadcast interviews.`,
      targetOutlets: ['The Economic Times', 'Mint', 'CNBC-TV18', 'NDTV Profit'],
      suggestedAngle: 'Commercial Scalability & National Market Expansion Hook',
    };
  } else if (action === 'refine_goals') {
    const raw = payload.rawText?.trim() || '';
    return {
      refinedGoals: raw
        ? `Execute a multi-stage strategic communications campaign for ${company}: secure an exclusive Tier-1 embargo feature in The Economic Times / Mint, establish scientific/operational authority, and position executive leadership as the defining voice in ${service}.`
        : `Drive market leadership for ${company} through coordinated Tier-1 national media placements across print, digital, and broadcast networks, validating key commercial achievements.`,
      targetOutlets: ['The Economic Times', 'Mint', 'CNBC-TV18', 'NDTV Profit', 'YourStory'],
      suggestedAngle: 'Category Dominance & Industry Multiplier Hook',
    };
  } else {
    // generate_dossier
    const executiveName = payload.name || 'Executive Leader';
    return {
      executiveSummary: `Strategic consultation roadmap prepared for ${executiveName} (${company}) focusing on ${service} with Tier-1 national media execution.`,
      agendaItems: [
        `Core Narrative Audit: Distilling ${company}'s unfair competitive differentiator for national business editors.`,
        'Bureau & Wire Deployment: Orchestrating simultaneous briefings across Amravati, Akola, Pune, and Nashik bureaus.',
        'Spokesperson Broadcast Readiness: Live broadcast interview training and quote arbitration for CNBC-TV18 & NDTV Profit.',
        'Embargo Sequencing & Risk Protocol: Comprehensive SEBI / corporate compliance and leak mitigation.',
      ],
      targetMediaPlacements: [
        'The Economic Times (Tech & Corporate Bureau)',
        'Mint (Prime Enterprise Feature)',
        'CNBC-TV18 (Broadcast CEO Interview)',
        'NDTV Profit / YourStory (Leadership Profile)',
      ],
      recommendedAngle: 'National Industrial Scale & Enterprise Adoption Hook',
      embargoTimeline: 'T-14 Days: Asset Assembly & Fact-Checking; T-48 Hours: Confidential Embargo Briefings; Day 0: National Wire Broadcast & Digital Blitz.',
      preCallChecklist: [
        'High-resolution executive portrait and leadership bio',
        'Audited operational milestones or verified investment figures in ₹ Cr',
        'List of Tier-1 target publications prioritized by management',
      ],
    };
  }
}
