...
export const AdminDashboard: React.FC = () => {
  ...
  const handleCreatePressRelease = async (e: React.FormEvent) => {
    ...
    try {
      await api.createPressRelease({
        ...newPress,
        mediaContact: {
          name: 'GSRelation Media Desk',
          email: 'press@gsrelation.in',
          phone: '+91 88059 15047',
        },
      });
      await refreshData();
      await loadAdminData();
  ...
  // If not authenticated, render login page after all hooks have executed unconditionally
  if (!isAdminAuthenticated) {
  ...
  return (
    ...
    >
      ...
      <header
        className={`text-white sticky top-0 z-40 shadow-xl border-b transition-colors duration-300 ${
          isDark ? 'bg-[#050B17] border-blue-900/50' : 'bg-[#1C473A] border-[#9B783E]/30'
        }`}
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              ...
              <div
                className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-md text-[10px] font-semibold uppercase tracking-widest border ${
                  isDark
                    ? 'bg-blue-600/30 text-blue-300 border-blue-500/40'
                    : 'bg-white/15 text-[#E0C078] border-white/20'
                }`}
              >
                <ShieldCheck className={`w-3.5 h-3.5 ${isDark ? 'text-blue-400' : 'text-[#E0C078]'}`} />
          ...
          <div className="flex items-center gap-3">
            {/* User Badge */}
            <div className="hidden md:flex items-center gap-2.5 px-3 py-1.5 rounded-md bg-white/10 border border-white/15 text-xs">
              <div className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
              <div className="text-left">
                <div className="font-semibold text-white leading-none">Gsrelation.admin</div>
                <div className="text-[10px] text-white/70 font-mono mt-0.5">Super Admin • IST</div>
              </div>
            </div>
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-md bg-white/10 hover:bg-white/20 border border-white/15 text-xs text-white transition-colors"
              title="Return to Public Agency Website"
            >
              <Globe className={`w-3.5 h-3.5 ${isDark ? 'text-blue-400' : 'text-[#E0C078]'}`} />
            ...
            <button
              ...
              onClick={logoutAdmin}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-md bg-rose-600/20 hover:bg-rose-600/30 border border-rose-500/40 text-rose-200 text-xs font-semibold transition-colors"
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center gap-1 overflow-x-auto no-scrollbar border-t border-white/10 py-1">
          {[
